...
class ShapedAxisStream:
    ...
    def __init__(
        ...
        if isinstance(zaber_axis, Axis):
            ...
            if zaber_axis.lockstep_group_id > zaber_axis.device.settings.get("lockstep.numgroups") or zaber_axis is None:
    ...
    def move_relative(
        ...
        position_mm = self._primary_axis.settings.convert_from_native_units("pos", position_native, Units.LENGTH_MILLIMETRES)
        ...
        self.stream.disable()
        if isinstance(self.axis, Lockstep):
            self.stream.setup_live_composite(StreamAxisDefinition(self.axis.lockstep_group_id, StreamAxisType.LOCKSTEP))
        else:
        ...
        for segment in stream_segments:
            ...
            if (
                ...
                self.stream.set_max_tangential_acceleration(segment.accel, Units.ACCELERATION_MILLIMETRES_PER_SECOND_SQUARED)
            else:

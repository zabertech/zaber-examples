...
- [shaped_axis.py](shaped_axis.py) - Contains the `ShapedAxis` class, which allows the input shaper classes to be easily used with Zaber devices. It is recommended for most users to import this class into their code.
...
Helper files:
- [shaper_config.py](shaper_config.py) - Contains a `ShaperConfig` class to manage configuration and settings for the `ShapedAxis` or `ShapedLockstep` classes.
...
```
- The axis parameter is the [`Axis`](https://software.zaber.com/motion-library/api/py/ascii/axis) class instance from which the ShapedAxis is derived. The `ShapedAxis` exposes the underlying methods and properties from the `Axis` class.
...
- Shaped movement commands using `ShaperMode.DECEL` mode will adjust the [motion.decelonly](https://www.zaber.com/protocol-manual#topic_setting_motion_decelonly) setting. The `reset_deceleration()` method can be used to restore this value. No other trajectory settings are adjusted.
...
- `ShaperMode.STREAM`: Input shaper using [streams](https://software.zaber.com/motion-library/docs/guides/streams) to send a trajectory generated by convolving impulses with trapezoidal motion. The most basic shaper is the ZV shaper which is described in [Zero Vibration Shaping](https://www.zaber.com/articles/input-shaping-for-vibration-reduction#Zero_Vibration_Shaping)
  - Settings:
    ...
    - `stream_id`: Identifier number for the stream on the device. This is necessary if multiple streams are being used on a single device in order to avoid a conflict. For example, when performing input shaping in stream mode on two separate axes on a controller, each axis will need its own stream, so a different id needs to be specified when configuring each `ShapedAxis` class.
      - Default value: 1

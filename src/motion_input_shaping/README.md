...
- [shaped_axis_stream.py](shaped_axis.py) - Contains the `ShapedAxisStream` class to perform input shaping using [streams](https://software.zaber.com/motion-library/docs/guides/streams) to send a trajectory generated by convolving impulses with trapezoidal motion.
...
```python
shaped_axis_var = ShapedAxisStream(axis, resonant_frequency, damping_ratio, ShaperType.ZV, stream_id)
```
- The axis parameter is the [`Axis`](https://software.zaber.com/motion-library/api/py/ascii/axis) class instance from which the ShapedAxis is derived. The `ShapedAxis` exposes the underlying methods and properties from the `Axis` class.
- See [here](https://software.zaber.com/motion-library/docs/tutorials/code) for a basic tutorial on how to initialize the `Axis` class.
...
## Selecting ShapedAxisStream or ShapedAxis

There are two different classes available to perform input shaping and ideal one to use will depend on the system and application it is being used for. Details on the pros and cons for each method is provided below.
